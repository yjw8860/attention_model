    ...
    def saveBestModelWithLoss(self, loss_list, avg_loss, model_name='G'):
        if len(avg_loss) > 0:
            if min(avg_loss) > np.mean(loss_list):
                ...
                if model_name == 'G':
                    print(f'Validation loss is not improved(G_loss:{np.mean(loss_list)})')
                elif model_name == 'L':
                    print(f'Validation loss is not improved(L_loss:{np.mean(loss_list)})')
                else:
                    print(f'Validation loss is not improved(F_loss:{np.mean(loss_list)})')
